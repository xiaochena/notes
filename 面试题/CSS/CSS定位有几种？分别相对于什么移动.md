...
```
点击此处[查看DEMO](https://www.runoob.com/try/try.php?filename=trycss_position_static)
...
fixed定位不为元素预留空间，而是通过指定元素相对于屏幕视口（viewport）的位置来指定元素位置
...
```
点击此处[查看Demo](https://www.runoob.com/try/try.php?filename=trycss_position_fixed)
